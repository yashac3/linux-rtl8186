...
static const char version[] = DRV_NAME ": Realtek RTL8186 10/100 Ethernet driver v" DRV_VERSION "\n";
			      ...
			      DMA_DESC *desc)
{
	...
	if (rtl8186_start_xmit_internal(send_skb, rtl8186_priv(send_skb->dev), send_skb->dev) == NETDEV_TX_BUSY) {
...
static int rtl8186_rx_poll(struct napi_struct *napi, int budget) {
	...
	while (rx < budget) {
		...
		if ((status & (FirstFrag | LastFrag)) != (FirstFrag | LastFrag)) {
			rtl8186_rx_err_acct(cp, rx_tail, status, len);
			printk_ratelimited(KERN_WARNING "%s: Got fragmented packet from device. Dropping!\n", cp->dev->name);
			goto setup_new_skb;
		}
		if (status & (RxError)) {
			rtl8186_rx_err_acct(cp, rx_tail, status, len);
		...
		if (unlikely(!new_skb)) {
			printk_ratelimited(KERN_WARNING "%s: napi_alloc_skb failed!\n", cp->dev->name);
			cp->net_stats.rx_dropped++;
			new_skb = skb;
			goto setup_new_skb;
		}
...
static void rtl8186_init_hw(struct re_private *cp)
{
	...
	rtl8186_reset_hw(cp);
	RTL_W8(CMD, 0x2); /* RX checksum offload enable, VLAN de-tagging disable */
...
static void rtl8186_change_mac_address(struct net_device *dev, u8 *mac) {
	...
	spin_lock_irqsave(&cp->lock, flags); // TODO is this lock should really be here??
...
{
	struct re_private *cp = rtl8186_priv(dev);
	uint32_t mii_data = BIT(31) | (location << 16) | (phy_id << 26) | (value & 0xffff);
	unsigned long flags;
...
static int rtl8186_eth_probe(struct platform_device *pdev) {
	...
	pr_info("%s: %s at 0x%lx (irq = %d)\n",
			pdev->name, dev->name, dev->base_addr, dev->irq);
